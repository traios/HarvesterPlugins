...
def _update_harvest_source_object(context, data_dict):
 ...
 ##---------------update  job to mongodb--------
    #client=pymongo.MongoClient('83.212.122.164',17017)

    #job={"cat_url":str(source.url),"type":str(source.type),"id":str(source.id),"description":str(source.description),"frequency":str(source.frequency),"title":str(source.title)}
   # db=client.odm
   # collection=db.jobs
   # temp=collection.find_one({"cat_url":str(source.url)})
   # temp.update(job)
